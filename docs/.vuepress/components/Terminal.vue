    ...
    <div class="terminal__body" ref="bodyRef">
      ...
      <template v-if="renderedText">
        <div
          ...
          class="term-line"
          :class="{
            'term-line--cmd': line.type === 'command',
            'term-line--out': line.type === 'output',
            'term-line--blank': line.type === 'blank',
            'term-line--comment': line.type === 'comment'
          }"
        >
...
const computedTitle = computed(() =>
  props.title && props.title.trim() ? props.title : 'Terminal',
);
  ...
  nextTick(() => {
    ...
    if (s) {
      ...
      if (common !== Infinity && common > 0) {
        lines = lines.map((ln) =>
          ln.startsWith(' '.repeat(common)) ? ln.slice(common) : ln,
        );
      }
...
const parsedLines = computed(() => {
  ...
  if (!raw) {
    return [] as Array<{
      type: 'command' | 'output' | 'blank' | 'comment';
      raw: string;
      prompt?: string;
      command?: string;
      comment?: string;
...
function tokenizeCmd(cmd?: string): Array<{ text: string; class?: string }> {
  ...
  while (i < cmd.length) {
    ...
    if (flagMatch) {
      pushText(flagMatch[1]);
      pushText(flagMatch[2], 'hl-flag');
...
.term-cmd :deep(.hl-cmd) {
  color: var(--vp-c-brand-1);
  font-weight: 600;
}
.term-cmd :deep(.hl-flag) {
  color: var(--vp-c-text-2);
}
.term-cmd :deep(.hl-str) {
  color: var(--vp-c-warning-1, #e6a700);
}
.term-cmd :deep(.hl-op) {
  color: var(--vp-c-default-3, #888);
}
